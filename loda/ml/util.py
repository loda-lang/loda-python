...
def tokens_to_operation(type_tok: str, target_tok: str, source_tok: str):
    try:
        type = Operation.Type[type_tok.upper()]
        target = Operand(target_tok)
        source = Operand(source_tok)
...
def tokens_to_program(tokens: list[str]) -> Program:
    ...
    while i+2 < len(tokens):
        operation = tokens_to_operation(tokens[i], tokens[i+1], tokens[i+2])
...
def merge_programs(program_cache: ProgramCache, num_programs: int = -1, num_ops_per_sample: int = 1) -> Program:
    ...
    for id in ids:
        ...
        append_nops(merged, num_ops_per_sample)
        for op in program.operations:
